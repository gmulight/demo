<template>
  <div 
    ...
    <div class="card-list"
         v-if="items.length > 1"
         ref="card-list"
         :style="{ left: (-width*2) + 'px', height: height  + 'px',width: width*cItems.length  + 'px' }">
      <div class="card-item"
           v-for="(item, index) in cItems"
           :key="index"
           :ref="`card${index-2}`"
           :style="(index-2) === -1 && { transform: `translateX(-1px)`,height: height  + 'px',width: width  + 'px'  }">
        <slot :name="`card${index-2}`">
          <image :style="{ height: height  + 'px',width: width  + 'px' }"
                 :src="item.src"></image>
        </slot>
      </div>
    </div>
...
import BindEnv from 'lighting-ui/packages/utils/bind-env';
...
export default {
  ...
  computed: {
    // isCreator () {
    //     console.log('weex.supports'+weex.supports)
  ...
  methods: {
    ...
    onPanMove (e) {
      ...
      const currentCardLeft = this.currentIndex * this.cardS.width;
    ...
    panEnd (e) {
      ...
      if (Utils.env.isWeb()||process.env.RUNTIME === "miniapp") {
    ...
    bindExp (element) {
      if (element && element.ref) {
        ...
        const dist = currentIndex * cardS.width;
        const listEl = this.$refs['card-list'];
        // 卡片容器
        const props = [{
          element: listEl.ref,
    ...
    slideTo (originIndex, selectIndex) {
      console.log(originIndex, selectIndex);
      console.log()
      const { cardS, timingFunction } = this;
      const listEl = this.$refs['card-list'];
      if (!listEl) { return; }
      const dist = selectIndex * cardS.width;
      // 卡片容器
      listEl && animation.transition(listEl, {
        styles: {
    ...
    checkNeedReset () {
      ...
      listEl && animation.transition(listEl, {
        styles: {
          transform: `translateX(${-this.currentIndex * cardS.width}px)`
        },
        duration: 0.00001,
        timingFunction
      });
    ...
    //   const idx = this.isCreator ? e.position : e;
      this.$emit('fmSliderItemClicked', e);
    }
  },
  mounted () {
    setTimeout(() => {
