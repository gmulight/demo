<template>
  <div 
    ...
    <div class="card-list"
         v-if="items.length > 1"
         ref="card-list"
         :style="{ left: -(cardS.width * 2) + 'px', height: height  + 'px',width: width  + 'px' }">
      <div class="card-item"
           v-for="(item, index) in cItems"
           :key="index"
           :ref="`card${index-2}`"
           :style="(index-2) === -1 && { transform: `translateX(-12px)`,height: height  + 'px',width: width  + 'px'  }">
        <slot :name="`card${index-2}`">
          <image :style="{ height: height  + 'px',width: width  + 'px' }"
                 :src="item.src" draggable="false" 
